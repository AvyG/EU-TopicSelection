...
author: "Avelyn Garcia, Stefan Panggabean, Chris Butcher"
date: "31-05-2023"
output:
...
In the context of political and international analysis,  studying the priorities of countries and political organizations yields fundamental insights that help to explain past and current events and allow predictions of future steps, which can impact millions of people. Motivated by recent events in Europe, we decided to contribute to these insights by examining the EU State of the Union speeches using unsupervised machine learning techniques. Our goal was to illustrate the changing priorities of the European Union over time.

We applied LDA to the eleven speeches available from 2010 onwards. We first downloaded the PDF versions of the speeches in English and scraped the main theme description of the speeches from the official website for model validation. After obtaining the speeches, we preprocessed them by cleaning uninformative text (including punctuation), lemmatizing them, and removing a custom set of stopwords. We found some words were so prevalent in the text that they could confuse our model. After that, we used the average of the Perplexity and Coherence metrics after 10 iterations to determine the number of topics (k).

In terms of the progression of the topics over time, we found that from 2010 to 2013, the selected topics primarily focused on the economy, which then shifted in 2015 to topics about refugees and Greece. During 2016-2018, it was challenging to find meaning in the selected topics from our model, as these topics contained mostly generic words. However, in 2020 and 2021, our selected topics included words like 'global,' 'world,' and some uplifting words like 'support' and 'together,' clearly referencing the COVID-19 pandemic period. Finally, in 2022, our model selected a unique and clear topic featuring words like 'Ukraine,' 'war,' and 'democracy,' referencing the war in Ukraine and perceived threats to Western democracies.

As previously stated in the validation section, there are multiple challenges to address regarding our model. Some of these challenges involve a more thorough cleaning of the text, while others question the selected hyperparameters and the appropriateness of our model. We are aware of these difficulties and look forward to addressing them in future improvements. Also, for the sake of model validation, we believe our study could benefit from using other techniques, like DTM, to evaluate the topics selected by our model.


#### 8. Elevator Pitch

Our group analyzes the big themes from all the State of the Union speeches of the European Union to see how policies shift over time, from 2010 to 2022. To do this, we extract the most important keywords of each speech in text format by using Latent Dirichlet Allocation and use it to infer concerns deemed paramount to address by the European Union. To do this, our group decided the range of topic numbers and sought to minimize metrics of word distribution prediction and semantic cohesiveness of topics based on simulations. In general, our group obtained that in spite of small issues on interpreting results, the model was able to identify the main topics of each speech after checking with the keywords from the European Union site.
