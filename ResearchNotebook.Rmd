...
The text in between [] are either grammatical corrections or adding text to clarify the example. If there are any question, please send an email to any of the students (for example: avelynfernanda.garciaaraya@student.kuleuven.be) who can answer in more detail the questions. 
...
#install.packages(c("rvest","tidyverse","httr","pdftools","quanteda", "wordcloud","topicmodels","textstem", "tidytext"))
...
Beside the string mention earlier we also clean the text from punctuation and extra spaces to the save the clean textun our dataframe.
There are two issues we would like to address here:

- The speech of 2016 has some images with text. This text is readed by the read_pdf command. We try our best to remove it, but it was not possible without eliminate part of the speech that we want to analyze. For this reason we decided to keep this extra text since for our purposes it is not that inconvenient. The images are related to the speech, so in term of finding the main topic of the speech the images should not bias the results.
- We decided to eliminate certain words that are repetitive and do not give information to our purposes. Addresses like: Mr President and Honourable members are repited so much in the speeches that topic models could confuse them with actual important topics.
- We also decided to eliminates the contraction 's since, again thinking about topic selection, they do not provide a lot of information.

To finish preprocessing our texts, we tokenize, lemmatize and remove stopwords to improve the results.

```{r textstem}

custom_stopwords = c(stopwords("en"), "europe", "european")
...
Before implementing topic models, we should like to see in more detail the text data we extracted from the speeches.
...
We can see that the most frequent words are not really informative. This motive us to remove some of the most common non informative words of the text 

```{r}

custom_stopwords = c(stopwords("en"), "europe", "european","eu","much","can","state","union","make","year","also","must","need","commission", "member")

new_tokens <- my_tokens %>% 
              quanteda::tokens_remove(custom_stopwords)
...
```
From the could of word we can see that there seem to be a small differentiation between speeches.
...
LDA (add description of lda)

[add search of hyperparameters]

```{r LDA}
# LDA for all speechs together

set.seed(0)

# Create vector for k and alpha
k_lda <- seq(3,10)
alpha_lda <- c(10,5,1,0.1,0.01,0.001)

#Create a list to save the models
lda_models <- list()

# Nest a loop to test each configuration of hyperparameters
for (k in k_lda){
